...
class MinifigureTotals
{
    ...
    private function base(): Builder
    {
        ...
        // «Потеряны» — про саму фигурку: набору её не хватает. Число это уже
        // считается и показывается бейджем на карточке, а отобрать по нему было
        // нельзя — увидеть пропажу можно было, только открыв набор.
        //
        // Области, как у деталей, здесь не нужно: у экземпляра, которым владеют
        // отдельно, «потеряна целиком» не бывает — такой фигурки в коллекции
        // просто нет, и правильный ответ удалить экземпляр, а не помечать его.
        if (! empty($this->filters['lost'])) {
            $query->havingRaw('lost > 0');
        }
